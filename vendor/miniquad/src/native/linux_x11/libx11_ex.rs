...
impl LibX11 {
    ...
    pub unsafe fn create_window(
        ...
        // GNOME and other Linux desktops classify an X11 window when it is
        // mapped. Publish identity and pixels here, after XCreateWindow but
        // before show_window/XMapWindow, so the first classification is final.
        let string_atom = (self.XInternAtom)(display, b"STRING\0".as_ptr().cast(), false as _);
        let wm_class_atom = (self.XInternAtom)(display, b"WM_CLASS\0".as_ptr().cast(), false as _);
        let wm_class = b"deitygb\0DeityGB\0";
        (self.XChangeProperty)(
            display,
            window,
            wm_class_atom,
            ...
            PropModeReplace,
            wm_class.as_ptr(),
            wm_class.len() as libc::c_int,
        );

        if let Some(icon) = &conf.icon {
            let cardinal = (self.XInternAtom)(display, b"CARDINAL\0".as_ptr().cast(), false as _);
            let net_wm_icon =
                (self.XInternAtom)(display, b"_NET_WM_ICON\0".as_ptr().cast(), false as _);
            let mut pixels: Vec<libc::c_ulong> = Vec::with_capacity(2 + 64 * 64);
            pixels.extend([64, 64]);
            pixels.extend(icon.big.chunks_exact(4).map(|pixel| {
                (libc::c_ulong::from(pixel[3]) << 24)
                    | (libc::c_ulong::from(pixel[0]) << 16)
                    | (libc::c_ulong::from(pixel[1]) << 8)
                    | libc::c_ulong::from(pixel[2])
            }));
            (self.XChangeProperty)(
                display,
                window,
